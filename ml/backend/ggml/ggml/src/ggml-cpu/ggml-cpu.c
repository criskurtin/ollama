...
static inline void ggml_opt_mul_mat_block_avx512(
    ...
    // Constant for offset correction (since we store as value-128)
    const __m512i offset_128 = _mm512_set1_epi32(128);
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j += 16) {
            ...
            __m512i acc_prod = _mm512_setzero_si512();  // sum of A_stored * B_stored
            __m512i acc_a = _mm512_setzero_si512();     // sum of A_stored
            __m512i acc_b = _mm512_setzero_si512();     // sum of B_stored
            ...
            for (int k = 0; k < K; k += 4) {
                ...
                for (int kk = 0; kk < 4 && k + kk < K; kk++) {
                    ...
                    // Accumulate for bias correction
                    // We need: sum((a+128)*(b+128)) = sum(a*b) + 128*sum(a) + 128*sum(b) + K*128*128
                    acc_a = _mm512_add_epi32(acc_a, a_vec_32);
                    acc_b = _mm512_add_epi32(acc_b, b_vec);

                    // Multiply and accumulate the main product
                    __m512i prod = _mm512_mullo_epi32(a_vec_32, b_vec);
                    acc_prod = _mm512_add_epi32(acc_prod, prod);
                }
            }

            // Apply bias correction: result = sum(a*b) + 128*sum(a) + 128*sum(b) + K*16384
            __m512i acc_a_scaled = _mm512_mullo_epi32(acc_a, offset_128);
            __m512i acc_b_scaled = _mm512_mullo_epi32(acc_b, offset_128);
            __m512i bias_constant = _mm512_set1_epi32(K * 128 * 128);

            __m512i corrected = _mm512_add_epi32(acc_prod, acc_a_scaled);
            corrected = _mm512_add_epi32(corrected, acc_b_scaled);
            corrected = _mm512_add_epi32(corrected, bias_constant);

            // Convert to float and apply scaling
            __m512 result = _mm512_cvtepi32_ps(corrected);
            result = _mm512_mul_ps(result, scale_vec);

            // Apply zero-point correction
            // After dequantization, the full formula is:
            // C[i][j] = sum_k((a_q*scale_a + zero_a) * (b_q*scale_b + zero_b))
            //         = sum_k(a_q*b_q)*scale_a*scale_b              [already in result]
            //           + zero_b*scale_b*sum_k(a_q*scale_a)         [term 2]
            //           + zero_a*scale_a*sum_k(b_q*scale_b)         [term 3]
            //           + K*zero_a*zero_b                           [term 4]
            //
            // where a_q = a_stored + 128, b_q = b_stored + 128
            //
            // Term 2: zero_b * scale_b * sum_k((a_stored+128) * scale_a)
            //       = zero_b * scale_b * scale_a * sum_k(a_stored+128)
            //       = zero_b * scale_b * scale_a * (sum(a_stored) + K*128)
            // Term 3: zero_a * scale_a * sum_k((b_stored+128) * scale_b)
            //       = zero_a * scale_a * scale_b * (sum(b_stored) + K*128)
            // Term 4: K * zero_a * zero_b

            const float zero_a = A->zero_point;
            const float zero_b = B->zero_point;
            const float scale_a = A->scale;
            const float scale_b = B->scale;

            // Convert accumulators to float for the zero-point terms
            __m512 acc_a_float = _mm512_cvtepi32_ps(acc_a);
            __m512 acc_b_float = _mm512_cvtepi32_ps(acc_b);

            // Term 2: b_zero * a_scale * sum_k(a_stored + 128)
            //       = b_zero * a_scale * (sum(a_stored) + K*128)
            __m512 term2 = _mm512_set1_ps((float)K * 128.0f);
            term2 = _mm512_add_ps(acc_a_float, term2);
            term2 = _mm512_mul_ps(term2, _mm512_set1_ps(zero_b * scale_a));

            // Term 3: a_zero * b_scale * sum_k(b_stored + 128)
            //       = a_zero * b_scale * (sum(b_stored) + K*128)
            __m512 term3 = _mm512_set1_ps((float)K * 128.0f);
            term3 = _mm512_add_ps(acc_b_float, term3);
            term3 = _mm512_mul_ps(term3, _mm512_set1_ps(zero_a * scale_b));

            // Term 4: K * zero_a * zero_b
            __m512 term4 = _mm512_set1_ps((float)K * zero_a * zero_b);

            // Add all zero-point correction terms
            result = _mm512_add_ps(result, term2);
            result = _mm512_add_ps(result, term3);
            result = _mm512_add_ps(result, term4);
